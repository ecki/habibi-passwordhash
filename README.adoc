...
While there are password hash formats like BCRYPT (`$2y$`) or SCRYPT (`$scrypt$`) or the new winner of the password hash competition Aragon (`$argon`) password hashes
based on PBKDF2 are still in high demand. The main reason for this is, that NIST endorses PBKDF2 with HMAC and endorsed hash function in SP 800
...
* Uses PBKDF2 (RFC xxxx) with HMac (RFC xxxx) and SHA2 512.
* Supports variable number of iterations and salt bytes
* Specifies UTF-8 encoding for the bytes
